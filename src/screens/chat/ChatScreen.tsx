...
export default function ChatScreen({ route, navigation }: any) {
  ...
  const sendMessage = async () => {
    ...
    if (snap.exists()) {
      ...
      await setDoc(chatRef, { lastMessage: text, updatedAt: serverTimestamp(), participants: [] });
    }
  };
          ...
          return (
            <View style={[styles.bubbleWrap, isMine ? styles.wrapMine : styles.wrapTheirs]}>
              <View style={[styles.bubble, isMine ? styles.bubbleMine : styles.bubbleTheirs]}>
                <Text style={[styles.bubbleText, { color: isMine ? theme.colors.bg : theme.colors.text }]}>
                  {item.text}
                </Text>
              </View>
            </View>
...
const styles = StyleSheet.create({
  ...
  sendIcon: { color: theme.colors.bg, fontSize: 16, fontWeight: "800" },
});